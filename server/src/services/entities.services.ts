    ...
    override async findAll(params: PaginationParams = {}): Promise<PaginatedResponse<GlobalType>> {
        ...
        try {
            ...
            const [rows] = await pool.execute(
                `SELECT t.*, (
                    SELECT COUNT(DISTINCT id) FROM items WHERE global_type = t.id
                ) as item_count
...
const converted = this.convertDatabaseValues(items[0]) as WeaponItemResponse;
    ...
    /**
     * Отримати записи зброї за територією
     */
    async findByTerritory(territoryId: number, params: PaginationParams, filterParams?: any): Promise<PaginatedResponse<WeaponItemResponse>> {
        const {
            page = 1,
            limit = 20,
            sortBy = 'id',
        ...
        try {
            ...
            const [countResult] = await pool.execute(
                `SELECT COUNT(DISTINCT it.item_id) as total FROM item_territories it 
                 JOIN items i ON i.id = it.item_id${joinClause} 
                 WHERE ${whereClause}`,
                queryParams
            ) as [RowDataPacket[], any];
            ...
            console.error('Error in findByTerritory:', error);
            throw error;
        }
    }
